...
bool ScriptHotReloadManager::LoadScripts()
{
    ...
    // 2.ファクトリー関数の取得
    mCurrentDll.CreateFunc = (CreateScriptComponentFunc)GetProcAddress(
        mCurrentDll.hDll, "CreateScriptComponent");

    mCurrentDll.UnregisterFunc = (UnregisterScriptComponentsFunc)GetProcAddress(
        mCurrentDll.hDll, "UnregisterScriptComponents");
    ...
    for (ActorObject* actor : SceneManager::GetAllActorsInCurrentScene())
    {
        ...
        for (Component* comp : actor->GetComponents())
        {
            if (ScriptComponent* script = dynamic_cast<ScriptComponent*>(comp))
            {
                ...
                mSavedState[actor].push_back({script->GetName(), compData});
    ...
    */
    ...
    for (const auto& pair : mSavedState)
    {
        ...
        for (const auto& savedScript : pair.second)
        {
            ...
            Component* newComp =
                mCurrentDll.CreateFunc(savedScript.ClassName.c_str(), entity);
            // 状態をデシリアライズ
            newComp->Deserialize(
                savedScript.Data); // 新しいDLLのDeserializeを呼び出し
            // newCompをentityにアタッチ
            ScriptComponent* newScript =
                dynamic_cast<ScriptComponent*>(newComp);
            if (newScript)
            {
...
bool ScriptHotReloadManager::CheckForChanges()
{
    ...
    for (const auto& entry :
         std::filesystem::recursive_directory_iterator(mScriptsDirectory))
    {
        if (entry.is_regular_file())
        {
            ...
            if (path.ends_with(".cpp") || path.ends_with(".h"))
            {
                ...
                if (mKnownAssetTimestamps.find(path) ==
                    mKnownAssetTimestamps.end())
                {
                    assetsAddedOrModified.insert(path);
                    mKnownAssetTimestamps[path] = lastWriteTime;
                }
                else if (CompareFileTime(&lastWriteTime,
                                         &mKnownAssetTimestamps[path]) > 0)
                {
                    assetsAddedOrModified.insert(path);
                    mKnownAssetTimestamps[path] = lastWriteTime;
                }
...
int ScriptHotReloadManager::ExecuteAndWaitForProcess(const string& command)
{
    ...
    BOOL success = CreateProcessA(NULL,            // モジュール名
...
string ScriptHotReloadManager::FindMsBuildPath()
{
    ...
    const std::string vswhereCmd =
        GetVsWherePath() + " -latest -requires Microsoft.Component.MSBuild "
                           "-find MSBuild\\**\\Bin\\MSBuild.exe -nologo";
    ...
    if (!pipe)
    {
        throw std::runtime_error(
            "Failed to run vswhere.exe. Is Visual Studio Installer installed?");
    }
    ...
    {
        throw std::runtime_error("vswhere.exe did not return a path. Is Visual "
                                 "Studio (with MSBuild) installed?");
    }
...
bool ScriptHotReloadManager::ExecuteMsbuildAndReload()
{
    ...
    string msBuildCommand =
        msBuildPath + " " + targetPath +
        " /p:Configuration=Debug /p:Platform=x64 /p:TargetName=" + targetName;
    // コマンドの出力をログファイルにリダイレクト（ビルドエラーの詳細を確認するため）
    // string msBuildCommand = "cmd /c " + msBuildPath + " " + targetPath + "
    // /p:Configuration=Debug /p:Platform=x64 > build_log.txt 2>&1";
    ...
    if (buildResult != 0)
    {
        ...
        Debug::ErrorLog(
            "MSBuild failed. InGameProject.dll was NOT regenerated. : %d",
            buildResult);
