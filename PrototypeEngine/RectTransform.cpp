...
void RectTransform::SetOffsetX(float x) { mOffsetX = x; }

void RectTransform::SetOffsetY(float y) { mOffsetY = y; }
...
void RectTransform::ComputeWorldTransform()
{
    ...
    // ローカル座標計算
    Matrix4 localScale =
        Matrix4::CreateScale(mLocalScale.x, mLocalScale.y, mLocalScale.z);

    Matrix4 localRotate = Matrix4::CreateFromQuaternion(mLocalRotation);

    Matrix4 localTranslate = Matrix4::CreateTranslation(Vector3(
        mLocalPosition.x - mOffsetX, mLocalPosition.y - mOffsetY, 0.0f));

    mLocalTransform = localScale * localRotate * localTranslate;

    // 2. 自分自身の見た目を決定する「描画用ローカル行列」を作成
    // (ここで初めてWidth/Heightを掛ける)
    Matrix4 pixelScale =
        Matrix4::CreateScale(mRectScaleWidth * mLocalScale.x,
                             mRectScaleHeight * mLocalScale.y, 1.0f);
...
{
    if (childtransform &&
        childtransform->GetRectTransform()->GetParentActor() == mOwner)
    {
...
void RectTransform::SetParent(UIActorObject* newParent)
{
    ...
    if (mParentActor)
    {
        ...
        Matrix4 parentWorldInverse =
            mParentActor->GetRectTransform()->GetWorldTransform();
...
void RectTransform::RemoveParentActor() { SetParent(nullptr); }
...
void RectTransform::ActiveDirty() { mIsDirty = true; }

void RectTransform::Serialize(json& j) const { Component::Serialize(j); }

void RectTransform::Deserialize(const json& j) { mIsDirty = true; }
...
void RectTransform::DrawCustomGUI(const std::vector<PropertyInfo>& properties)
{
    ...
    if (ImGui::DragFloat("Width", &width, 0.1f, 0.0f, 0.0f,
                         "%.1f")) // 0.1fはドラッグの感度
    {
        SetScaleWidth(width);
    }
    ...
    if (ImGui::DragFloat("Height", &height, 0.1f, 0.0f, 0.0f,
                         "%.1f")) // 0.1fはドラッグの感度
    {
