...
DirectionalLightComponent::DirectionalLightComponent(Entity* owner)
    : Component(owner), mDirectionalLight()
{
...
void DirectionalLightComponent::OnUpdateWorldTransform()
{
    ...
    Vector3 lightDir =
        Vector3::Transform(baseDir, mActor->GetTransform()->GetRotation());
    lightDir.Normalize();
    ...
    mDirectionalLight.sDiffuseColor =
        Vector3::Lerp(nightDiffuse, dayDiffuse, dayFactor);

    // Specular（鏡面反射）
    Vector3 daySpecular   = Vector3(1.0f, 1.0f, 1.0f);
    Vector3 nightSpecular = Vector3(0.0f, 0.0f, 0.0f);
    mDirectionalLight.sSpecColor =
        Vector3::Lerp(nightSpecular, daySpecular, dayFactor);
...
void DirectionalLightComponent::Serialize(json& j) const
{
    ...
    j["Direction"]        = {mDirectionalLight.sDirection.x,
                             mDirectionalLight.sDirection.y,
                             mDirectionalLight.sDirection.z};
    j["DiffuseColor"]     = {mDirectionalLight.sDiffuseColor.x,
                             mDirectionalLight.sDiffuseColor.y,
                             mDirectionalLight.sDiffuseColor.z};
    j["AmbientColor"]     = {mDirectionalLight.sAmbientColor.x,
                             mDirectionalLight.sAmbientColor.y,
                             mDirectionalLight.sAmbientColor.z};
    j["AmbientIntensity"] = mDirectionalLight.sAmbientIntensity;
    j["SpecularColor"]    = {mDirectionalLight.sSpecColor.x,
                             mDirectionalLight.sSpecColor.y,
                             mDirectionalLight.sSpecColor.z};
}
...
void DirectionalLightComponent::Deserialize(const json& j)
{
    ...
    if (j.contains("DiffuseColor"))
    {
        ...
        mDirectionalLight.sDiffuseColor.Set(diffArray[0], diffArray[1],
                                            diffArray[2]);
    }
    if (j.contains("AmbientColor"))
    {
        auto ambArray = j["AmbientColor"];
        mDirectionalLight.sAmbientColor.Set(ambArray[0], ambArray[1],
                                            ambArray[2]);
    }
    ...
    if (j.contains("SpecularColor"))
    {
        ...
        mDirectionalLight.sSpecColor.Set(specArray[0], specArray[1],
                                         specArray[2]);
    }
}

void DirectionalLightComponent::DrawCustomGUI(
