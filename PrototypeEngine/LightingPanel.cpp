...
LightingPanel::LightingPanel(Renderer* renderer) : EditorWindow(renderer)
{
    mID = "LightingPanel";
}
...
void LightingPanel::Draw(float width, float height)
{
    ...
    if (ImGui::Begin(GetImGuiWindowID().c_str(), &mIsShow, flags))
    {
        // 1.ファイルパスの取得
        string currentPath =
            EngineWindow::GetRenderer()->GetSkyBoxRenderer()->GetLoadFilePath();
        static char pathBuffer[256];
        strncpy_s(pathBuffer, currentPath.c_str(), sizeof(pathBuffer));
        pathBuffer[sizeof(pathBuffer) - 1] = '\0';
        ImGui::Text("FilePath DragDropTarget");
        // 2.ファイルパスの入力フィールド
        ImGui::InputText("SkyBoxTexture File Path", pathBuffer,
                         sizeof(pathBuffer), ImGuiInputTextFlags_ReadOnly);
        ...
        if (ImGui::BeginDragDropTarget())
        {
                    ...
                    ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
            {
                ...
                if (EngineWindow::GetRenderer()->GetSkyBoxRenderer()->Load(
                        dropPath))
                {
                    EngineWindow::GetRenderer()
                        ->GetRunScene()
                        ->SetLoadSkyBoxTexturePath(dropPath);
                }
            }
