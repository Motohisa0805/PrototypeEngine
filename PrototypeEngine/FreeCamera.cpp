...
FreeCamera::FreeCamera(Entity* owner) : BaseCamera(owner), mUp(Vector3::UnitY)
{
    mName = "FreeCamera";
...
void FreeCamera::Initiailze() { MoveCameraView(); }
...
void FreeCamera::MoveCameraView()
{
    ...
    Vector3 viewForward =
        Vector3::Transform(mActor->GetTransform()->GetForward(),
                           mActor->GetTransform()->GetLocalRotation());
