...
IsLandPhysic::IsLandPhysic(PhysWorld* world) : mWorld(world), mActiveBodies() {}

void IsLandPhysic::BuildAndSolveIslands(std::vector<ContactManifold>& manifolds,
                                        float                         deltaTime)
{
    // 訪問フラグの管理
    ...
    for (Rigidbody* rb : mActiveBodies)
    {
        ...
        if (visited.count(rb) || rb->GetInverseMass() == 0.0f)
            continue;
...
void IsLandPhysic::SolveIsland(vector<Rigidbody*>&       islandBodies,
...
{
    if (rb->IsInActiveList())
        return; // すでにアクティブリストにある場合は追加しない
