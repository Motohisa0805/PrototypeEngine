...
void ProjectExplanationPanel::Initialize(float width, float height, ImTextureRef ref)
{
...
void ProjectExplanationPanel::Draw(float width, float height)
{
	...
	if (ImGui::Begin(GetImGuiWindowID().c_str(), &mIsShow, flags)) {
		...
		ImGui::SetWindowFontScale(1.15f);
		ImGui::Text("Move the selected file or folder by left-clicking and holding");
	}
	ImGui::End();
}
