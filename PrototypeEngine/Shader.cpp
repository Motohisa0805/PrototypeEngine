...
bool Shader::Load(const string& vertName, const string& fragName)
{
    ...
    if (!CompileShader(vertPath, GL_VERTEX_SHADER, mVertexShader) ||
...
void Shader::SetVector3Array(const string& name, const Vector3* array,
                             size_t count)
{
    GLuint loc = glGetUniformLocation(mShaderProgram, name.c_str());
    if (loc != -1)
    {
        glUniform3fv(loc, static_cast<GLsizei>(count),
                     reinterpret_cast<const GLfloat*>(array));
    }
}

void Shader::SetVector2Array(const string& name, const Vector2* array,
                             size_t count)
{
    GLuint loc = glGetUniformLocation(mShaderProgram, name.c_str());
    if (loc != -1)
    {
        glUniform2fv(loc, static_cast<GLsizei>(count),
                     reinterpret_cast<const GLfloat*>(array));
    }
}
...
void Shader::SetFloatArray(const string& name, const float* array, size_t count)
{
    ...
    if (loc != -1)
    {
        glUniform1fv(loc, static_cast<GLsizei>(count), array);
    }
}
...
bool Shader::CompileShader(const string& fileName, GLenum shaderType,
                           GLuint& outShader)
{
    // Open file
