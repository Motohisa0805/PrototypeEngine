...
const filesystem::path EditorSettingsManager::SETTEINGS_FILE_PATH =
    "Library/EditorSettings.json";
...
void EditorSettingsManager::CreateNewScene(
    const filesystem::path& filePath, const filesystem::path& selectingPath)
{
    string           uniqueName = "NewScene.json"; // 拡張子付きで初期化
    filesystem::path targetFolder =
        selectingPath; // 現在右クリックしているパス（フォルダ）
...
void EditorSettingsManager::ProcessScriptDelete(const filesystem::path& path)
{
    ...
    {
        filesystem::path cppPath =
            scriptPath.parent_path() / (scriptPath.stem().string() + ".cpp");
    ...
    {
        filesystem::path hPath =
            scriptPath.parent_path() / (scriptPath.stem().string() + ".h");
