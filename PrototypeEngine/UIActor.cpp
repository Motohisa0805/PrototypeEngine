...
UIActorObject::UIActorObject(uint64_t id) : Entity(id), mIsCanvas(false)
{
...
UIActorObject::UIActorObject(BaseScene* scene) : Entity(scene), mIsCanvas(false)
{
    mRectTransform = new RectTransform(this);
}
...
void UIActorObject::Serialize(json& j) const
{
    ...
    // ローカルの値を保存する
    j["LocalPosition"] = {mRectTransform->GetLocalPosition().x,
                          mRectTransform->GetLocalPosition().y,
                          mRectTransform->GetLocalPosition().z};
    j["LocalRotation"] = {mRectTransform->GetLocalRotation().w,
                          mRectTransform->GetLocalRotation().x,
                          mRectTransform->GetLocalRotation().y,
                          mRectTransform->GetLocalRotation().z};
    j["LocalScale"]    = {mRectTransform->GetLocalScale().x,
                          mRectTransform->GetLocalScale().y,
                          mRectTransform->GetLocalScale().z};
...
void UIActorObject::Deserialize(const json& j)
{
    ...
    mRectTransform->SetLocalPosition(Vector3(
...
void UIActorObject::LoadParentByLoadScene()
{
    ...
    {
        mRectTransform->SetParent(mGame->GetUIActorManager()->FindActorByID(
            mRectTransform->GetParentID()));
    }
}
...
Entity* UIActorObject::Clone()
{
    ...
    clone->GetRectTransform()->SetLocalPosition(
        this->GetRectTransform()->GetLocalPosition());
    clone->GetRectTransform()->SetLocalRotation(
        this->GetRectTransform()->GetLocalRotation());
    clone->GetRectTransform()->SetLocalScale(
        this->GetRectTransform()->GetLocalScale());
