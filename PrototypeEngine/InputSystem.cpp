...
void InputSystem::Update()
{
    ...
    for (int i = 0; i < SDL_GAMEPAD_BUTTON_COUNT; i++)
    {
        mState.Controller.mCurrButtons[i] =
            SDL_GetGamepadButton(mController, SDL_GamepadButton(i));
    }

    // Triggers
    mState.Controller.mLeftTrigger = Filter1D(
        SDL_GetGamepadAxis(mController, SDL_GAMEPAD_AXIS_LEFT_TRIGGER));
    mState.Controller.mRightTrigger = Filter1D(
        SDL_GetGamepadAxis(mController, SDL_GAMEPAD_AXIS_RIGHT_TRIGGER));

    // Sticks
    x = SDL_GetGamepadAxis(mController, SDL_GAMEPAD_AXIS_LEFTX);
    y = -SDL_GetGamepadAxis(mController, SDL_GAMEPAD_AXIS_LEFTY);
    mState.Controller.mLeftStick = Filter2D(x, y);

    x = SDL_GetGamepadAxis(mController, SDL_GAMEPAD_AXIS_RIGHTX);
    y = -SDL_GetGamepadAxis(mController, SDL_GAMEPAD_AXIS_RIGHTY);
...
void InputSystem::SetGameMouseMode(MouseMode mode)
{
    if (mode == MouseMode::RelativeMouse)
    {
        ...
        // --- 非表示にする瞬間 ---
        SDL_GetMouseState(&mState.Mouse.mMousePos.x,
                          &mState.Mouse.mMousePos.y); // 現在位置を保存
