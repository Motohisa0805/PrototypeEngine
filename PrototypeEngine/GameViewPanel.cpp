...
GameViewPanel::GameViewPanel(Renderer* renderer)
    : EditorWindow(renderer), mIsDebugStatesFrag(false)
{
    mID = "GameView";
}
...
void GameViewPanel::Draw(float width, float height)
{
    ...
    {
        // デバッグモード切り替えボタン
        ImGuiHelper::FragTextButton("State:", ImVec2(0.0f, 0.0f),
                                    mIsDebugStatesFrag);
        ...
        if (mRenderer->GetGameSceneViewEditor()->NeedsResize(
        ...
        // 描画処理
        //  ウィンドウサイズに合わせて描画
        ImGui::Image((ImTextureID)(intptr_t)mRenderer->GetGameSceneViewEditor()
                         ->GetSceneColorTex(),
                     winSize, ImVec2(0, 1), // uv0 (上下反転に注意)
                     ImVec2(1, 0)           // uv1
        );
        ...
        if (mIsDebugStatesFrag)
        {
            ...
            ImVec2 imageTopLeft =
                ImVec2(currntCursorPos.x, currntCursorPos.y - winSize.y);

            // 1.オーバーレイウィンドウの表示位置を計算
            ImVec2 overlayPos = ImVec2(imageTopLeft.x + winSize.x - padding,
            ...
            // 5.オーバーレイウインドウの描画開始
            if (ImGui::Begin("StatesOverlay", &mIsDebugStatesFrag,
                             overlayFlags))
            {
                // タイトル
                ...
                int vertices =
                    SceneManager::GetCurrentRunScene()->GetSceneAllVertices();
