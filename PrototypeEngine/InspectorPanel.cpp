...
void InspectorPanel::ComponentSelectorDraw(Entity* selectedActor)
{
    ...
    vector<string> componentNames =
        ComponentFactory::GetRegisteredComponentNames();
    ...
    for (const string& compName : componentNames)
    {
        ...
        if (ImGui::MenuItem(compName.c_str()))
        {
            ...
            Component* newComp =
                ComponentFactory::CreateComponent(compName, selectedActor);
            if (newComp)
            {
                selectedActor->AddComponent(newComp);
                // メッシュとコライダーの依存性が解決したら処理
                selectedActor->OnComponentAdded(
                    newComp); // ← ActorObject側で実装する
...
void InspectorPanel::DrawComponentProperties(Component*          comp,
                                             const PropertyInfo& prop)
{
    // Componentインスタンスの先頭アドレス + オフセット =
    // メンバ変数の値を編集する
