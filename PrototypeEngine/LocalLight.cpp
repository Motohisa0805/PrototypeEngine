...
void LocalLight::DrawCustomGUI(const std::vector<PropertyInfo>& properties)
{
	...
	if (ImGui::BeginCombo("LightType", GetLightComponentName(mLightType).c_str()))
	{
		for (uint32_t i = 0; i < LightType::Count; ++i) {
			LightType tag = static_cast<LightType>(i);
			bool isSelected = (mLightType == tag);

			if (ImGui::Selectable(GetLightComponentName(tag).c_str(), isSelected))
			{
				mLightType = tag;
			}
		}
	...
	else if (mLightType == LightType::Area) {
