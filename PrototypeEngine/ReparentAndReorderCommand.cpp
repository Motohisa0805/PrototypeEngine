...
ReparentAndReorderCommand::ReparentAndReorderCommand(Entity* target, Entity* newParent, size_t toIndex)
	...
	, mFromIndex(0)
{
	...
	// UIActorか確認
		...
		// 移動前の親のIDを記憶
		UIActorObject* oldParent = uiActorPtr->GetRectTransform()->GetParentActor();
		mOldParentID = oldParent ? oldParent->GetID() : 0;
	}
...
void ReparentAndReorderCommand::Execute()
{
	...
	UIActorObject* uinewParent = (mNewParentID != 0) ? uiActorManager->FindActorByID(mNewParentID) : nullptr;
...
void ReparentAndReorderCommand::Undo()
{
	...
	UIActorObject* uitarget = uiActorManager->FindActorByID(mTargetID);
	UIActorObject* uioldParent = (mNewParentID != 0) ? uiActorManager->FindActorByID(mOldParentID) : nullptr;
