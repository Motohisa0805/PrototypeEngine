...
Font::Font() : mGame(SceneManager::GetCurrentRunScene()) {}
...
bool Font::Load(const string& fileName)
{
    ...
    vector<int> fontSizes = {8,  9,  10, 11, 12, 14, 16, 18, 20, 22,
...
Texture* Font::RenderFreeText(const string& textKey, const Vector3& color,
                              int pointSize)
{
    ...
    if (iter != mFontData.end())
    {
        ...
        SDL_Surface* pixelsSurf = TTF_RenderText_Blended(
            font, actualText.c_str(), actualText.length(), sdlColor);
        if (!pixelsSurf)
        {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "Failed to render text: %s", SDL_GetError());
            return nullptr;
        }

        // ピクセルフォーマットを RGBA32 に変換
        SDL_Surface* surf =
            SDL_ConvertSurface(pixelsSurf, SDL_PIXELFORMAT_RGBA32);
        // 元のサーフェスは不要なので削除
