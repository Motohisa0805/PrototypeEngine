...
void Entity::Serialize(json& j) const
{
    ...
    for (const auto& component : mComponents)
    {
        ...
        component->Serialize(
            componentData); // 各コンポーネントのシリアライズメソッドを呼び出す
...
void Entity::Deserialize(const json& j)
{
    ...
    {
        // 静的状態タグを読み込む
        mStatic = static_cast<ActorInformation::StaticTag>(
            j.at("Static").get<uint32_t>());
    }
    ...
    if (j.contains("Components"))
    {
        for (const auto& componentData : j.at("Components"))
        {
            ...
            Component* newComponent =
                ComponentFactory::CreateComponent(type, this);
    ...
    if (meshRenderer = dynamic_cast<MeshRenderer*>(newComp))
    {
        ...
        if (BoxCollider* collider = GetComponent<BoxCollider>())
        {
            ...
            if (!meshRenderer->GetMeshs().empty())
            {
                ...
                collider->SetObjectOBB(
                    meshRenderer->GetMeshs()[0]->GetOBBBoxs()[0]);
            }
        }
    }
    // BoxColliderが追加された場合
