...
void Image::Load(string file)
{
    ...
    mUIActor->GetRectTransform()->SetScaleWidthAndHeight(
        static_cast<float>(mTextureRect.w), static_cast<float>(mTextureRect.h));
...
void Image::DrawTexture(Shader* shader)
{
    ...
    Matrix4 pixelScale =
        Matrix4::CreateScale(drawWidth * rt->GetLocalScale().x,
                             drawHeight * rt->GetLocalScale().y, 1.0f);

    Matrix4 localRotate = Matrix4::CreateFromQuaternion(rt->GetLocalRotation());

    Matrix4 localTranslate = Matrix4::CreateTranslation(
        Vector3(rt->GetLocalPosition().x + offsetX,
                rt->GetLocalPosition().y + offsetY, 0.0f));
    ...
    if (rt->GetParentActor())
    {
        drawTransform =
            drawTransform *
            rt->GetParentActor()->GetRectTransform()->GetWorldTransform();
    }
...
void Image::Serialize(json& j) const
{
    ...
    j["UVTransform"]   = {mUVTransform.x, mUVTransform.y, mUVTransform.z,
                          mUVTransform.w};
}
...
void Image::Deserialize(const json& j)
{
    ...
    {
        mUVTransform = Vector4(j["UVTransform"][0], j["UVTransform"][1],
                               j["UVTransform"][2], j["UVTransform"][3]);
    }
}
...
void Image::DrawCustomGUI(const std::vector<PropertyInfo>& properties)
{
    ...
    // 2.ファイルパスの入力フィールド
    ImGui::InputText("Image File Path", pathBuffer, sizeof(pathBuffer),
    ...
    if (ImGui::BeginCombo(
            "FillMethod",
            ImageInformation::GetFillMethodName(mFillMethod).c_str()))
    {
        for (uint32_t i = 0; i < 4; ++i)
        {
            ...
            if (ImGui::Selectable(
                    ImageInformation::GetFillMethodName(tag).c_str(),
                    isSelected))
            {
                mFillMethod = tag;
            }
