...
void GUIEditorManager::RenderImGui()
{
    ...
    ImGui::SetNextWindowPos(
        ImVec2(viewport->WorkPos.x, viewport->WorkPos.y + 50.0f));
    ImGui::SetNextWindowSize(
        ImVec2(viewport->WorkSize.x, viewport->WorkSize.y - 50.0f));
...
void GUIEditorManager::ApplyDefaultLayout_2by3()
{
    ...
    ImGuiID dock_id_remaining = ImGui::DockBuilderSplitNode(dockspace_id,
    ImGuiDir_Left, 0.2f, nullptr, &dock_id_hierarchy);

    ImGuiID dock_id_inspector;
    ImGuiID dock_id_center = ImGui::DockBuilderSplitNode(dock_id_remaining,
    ImGuiDir_Right, 0.25f, nullptr, &dock_id_inspector);

    ImGuiID dock_id_scene;
    ImGuiID dock_id_game = ImGui::DockBuilderSplitNode(dockspace_id,
    ImGuiDir_Down, 0.5f, nullptr, &dock_id_scene);
...
{
    auto it =
        std::find(mSceneViewPanels.begin(), mSceneViewPanels.end(), panel);
