...
void DebugGrid::GenerateGrid(float gridSize, int gridCount, const Vector3& center)
{
	//既存のVAOを削除
    if (mGridVAO)
    {
    ...
    for (int i = -half; i <= half; ++i)
    {
        ...
        // X軸に平行な線 (Z固定)
        verts.push_back(center.x + offset); verts.push_back(center.y); verts.push_back(center.z + start);
        verts.push_back(center.x + offset); verts.push_back(center.y); verts.push_back(center.z + end);

        // Z軸に平行な線 (X固定)
        verts.push_back(center.x + start); verts.push_back(center.y); verts.push_back(center.z + offset);
        verts.push_back(center.x + end);  verts.push_back(center.y); verts.push_back(center.z + offset);
    }
...
void DebugGrid::Draw(Shader* shader, const Matrix4& viewProj, const Vector3& cameraPos)
{
