...
void InspectorExplanationPanel::Initialize(float width, float height, ImTextureRef ref)
{
...
void InspectorExplanationPanel::Draw(float width, float height)
{
	...
	if (ImGui::Begin(GetImGuiWindowID().c_str(), &mIsShow, flags)) {
		...
		ImGui::Text("You can change it in the text box where the object name is written.");
		ImGui::NewLine();
		ImGui::SetWindowFontScale(1.15f);
		ImGui::Text("Changing a static flag");
		ImGui::SetWindowFontScale(1.0f);
		ImGui::Text("Please change the flag box to the right of the object name text box.");
		ImGui::NewLine();
		ImGui::SetWindowFontScale(1.15f);
